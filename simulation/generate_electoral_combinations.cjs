// simulation/generate_electoral_combinations.cjs
...
function generateElectoralCatalog() {
  ...
  console.log(`💾 Catálogo guardado en JSON de Simulación: ${jsonPath}`);

  // Asegurar que la carpeta src/data exista y guardar ahí también
  const srcDataDir = path.join(__dirname, '../src/data');
  if (!fs.existsSync(srcDataDir)) {
    fs.mkdirSync(srcDataDir, { recursive: true });
  }
  const frontendJsonPath = path.join(srcDataDir, 'electoral_scenarios.json');
  fs.writeFileSync(frontendJsonPath, JSON.stringify(scenarios, null, 2), 'utf-8');
  console.log(`💾 Catálogo inyectado en Frontend src/data: ${frontendJsonPath}`);
}
